...
def book_appointment(request, package_id):
    ...
    if request.method == "GET":
        ...
        if credentials_data:
            ...
            event_body = {
                "summary": f"Booking for {package.pkg_name}",
                "description": f"Customer: {request.user.customer_name}\n"
                               f"Contact: {request.user.contact_no}\n"
                               f"Package: {package.pkg_name}\n"
                               f"Created by Vow Planner",
                "start": {"date": selected_date},
                "end": {"date": selected_date},
            }

            created_event = service.events().insert(calendarId="primary", body=event_body).execute()
            new_event.google_event_id = created_event["id"]
            new_event.save()
